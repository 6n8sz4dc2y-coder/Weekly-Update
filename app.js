...
function build(){
 updateVersionDisplays();
 try { recomputeDashboardSets(DATA); } catch(e) { console.warn('Dashboard sets could not be recomputed', e); }
 ...
 document.getElementById('cdaSummary').innerHTML=[north,wy,south].filter(Boolean).map(r=>`<div class="leader-row"><div class="rank">●</div><div class="centre">${r.centre}<div class="mini">${fmt(r.qtr_total)} / ${fmt(r.qtr_target)} · To go ${fmt(r.to_go)}</div></div><div class="pct">${pct(r.qtr_target?r.qtr_total/r.qtr_target:0)}</div>${progress(r.qtr_target?r.qtr_total/r.qtr_target:0)}</div>`).join('');
...
function recomputeDashboardSets(data){
  const userSites = data.user_sites || ['Rochdale','Bury','Bolton','SQ','Bradford','Huddersfield','Silsden','Denton','Altrincham','Macclesfield','Stockport'];
...
function parseWeeklyWorkbook(wb, data){
  ...
  const centreRows = new Set(['Bolton','Bury','Rochdale','SQ','NORTH CDA','Bradford','Huddersfield','Silsden','WY CDA','Denton','Altrincham','Macclesfield','Stockport','SOUTH CDA']);
  const findRow = (label) => a.findIndex(r => String((r&&r[0])||'').trim().toUpperCase().includes(label));
  const ensureRow = (arr, centre, seed={}) => {
    let row=(arr||[]).find(r=>String(r.centre).trim().toLowerCase()===String(centre).trim().toLowerCase());
    if(!row){ row=Object.assign({centre}, seed); arr.push(row); }
    return row;
  };
  const updateAny = (arr, centre, patch, seed={}) => Object.assign(ensureRow(arr, centre, seed), patch);
  data.q3_regs = data.q3_regs || [];
  data.q3_used = data.q3_used || [];
  data.q3_fleet = data.q3_fleet || [];
  data.q3_fleet_monthly = data.q3_fleet_monthly || [];
  data.q3_non = data.q3_non || [];

  // Registrations counting: starts after REGISTRATIONS (COUNTING), ends before blank/non-counting section.
  const regStart = Math.max(3, findRow('REGISTRATIONS (COUNTING)') + 3);
  for(let i=regStart;i<a.length;i++){
    const r=a[i]; const centre=r&&r[0];
    if(!centre) break;
    if(String(centre).toUpperCase().includes('REGISTRATIONS')) break;
    if(!centreRows.has(centre)) continue;
    const patch={
    ...
    updateAny(data.q3_regs, centre, patch, {jul_total:0,aug_total:0,sep_total:0,qtr_total:0,qtr_target:0});
  }

  // Non-counting fleet.
  const nonHead = findRow('REGISTRATIONS (NON-COUNTING)');
  const nonStart = nonHead >= 0 ? nonHead + 3 : 19;
  for(let i=nonStart;i<a.length;i++){
    const r=a[i]; const centre=r&&r[0];
    if(!centre) break;
    if(String(centre).toUpperCase().includes('USED COUNTING')) break;
    ...
    updateAny(data.q3_non, centre, patch, {qtr_total:0,qtr_budget:0});
  }

  // Used counting volume.
  const usedHead = findRow('USED COUNTING VOLUME');
  const usedStart = usedHead >= 0 ? usedHead + 3 : 29;
  for(let i=usedStart;i<a.length;i++){
    const r=a[i]; const centre=r&&r[0];
    if(!centre) break;
    if(String(centre).toUpperCase().includes('CENTRE FLEET')) break;
    if(!centreRows.has(centre)) continue;
    ...
    updateAny(data.q3_used, centre, patch, {jul_counting:0,aug_counting:0,sep_counting:0,qtr_counting:0,qtr_target:0});
  }

  // Centre Fleet BCH.
  const fleetHead = findRow('CENTRE FLEET');
  const fleetStart = fleetHead >= 0 ? fleetHead + 3 : 43;
  for(let i=fleetStart;i<a.length;i++){
    const r=a[i]; const centre=r&&r[0];
    if(!centre) break;
    if(!centreRows.has(centre)) continue;
    const patch={regs:nval(r[1]),target:nval(r[2]),pct:pctval(r[3]),active_orders:nval(r[4])};
    updateAny(data.q3_fleet, centre, patch, {regs:0,target:0,pct:0,active_orders:0});
    updateAny(data.q3_fleet_monthly, centre, {bch_regs:patch.regs,bch_target:patch.target,active_orders:patch.active_orders}, {jul_fleet:0,aug_fleet:0,sep_fleet:0,qtr_fleet:0});
  }
